...
<head>
    ...
    <script type="text/javascript">
            ...
            $(".jian").click(function () {
                ...
                if (quantity == 1) {
                    if (confirm("您是否真要删除该条目？")) {
                        location = "/goods/cartServlet?method=batchDelete&cartItemIds=" + cartItemId;
                    }
                } else {
            ...
            */
            ...
            $.ajax({
                ...
                success: function (response) {
                    //1.修改数量
                    $("#" + id + "Quantity").val(response.quantity);
                    //2.修改小计
                    $("#" + id + "Subtotal").text(response.total);
                    //3.重新计算总计
                    showTotal()
                }
            });
        ...
        function batchDelete() {
            ...
            //3.指定location为cartServlet,参数method=batchDelete,cartItemIds=数组的toString
            location = "/goods/cartServlet?method=batchDelete&cartItemIds=" + array;
        }

        //结算
        function jiesuan() {

            //1.获取所有被选取的条目的id,放到数组中
            var array = new Array();
            $(":checkbox[name=checkboxBtn][checked=true]").each(function () {
                array.push($(this).val())
            });
            //2.把数组的值toString() ,赋值给cartItemIds这个hidden
                ...
                <tr align="center">
                    ...
                    <td align="left" width="70px">
                        <a class="linkImage" href="<c:url value='/jsps/book/desc.jsp'/>"><img border="0" width="54"
                                                                                              align="top"
                                                                                              src="<c:url value='${cart.book.image_b}'/>"/></a>
                    </td>
                    <td align="left" width="400px">
                        <a href="<c:url value='/jsps/book/desc.jsp'/>"><span>${cart.book.bname}</span></a>
                    </td>
                    <td><span>&yen;<span class="currPrice"
                                         id="${cart.cartItemId}CurrPrice">${cart.book.currPrice}</span></span></td>
                    <td>
                        <a class="jian" id="${cart.cartItemId}Jian"></a><input class="quantity" readonly="readonly"
                                                                               id="${cart.cartItemId}Quantity"
                                                                               type="text" value="${cart.quantity}"/><a
                            class="jia" id="${cart.cartItemId}Jia"></a>
                    </td>

                    <td width="100px">
                        <span class="price_n">&yen;<span class="subTotal"
                                                         id="${cart.cartItemId}Subtotal">${cart.total}</span></span>
                    </td>
                    <td>
